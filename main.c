...
int main(void) {
    ...
    while(1) {
        ...
        case MODE_SELECT:
            waitForVBlank();
            drawFullScreenImageDMA(trackModeSelection);
            if (KEY_JUST_PRESSED(BUTTON_A, currentButtons, previousButtons)) {
                currentAppState.useTrack = 1;
                state = SPEED_SELECT;
            } else if (KEY_JUST_PRESSED(BUTTON_B, currentButtons, previousButtons)) {
                currentAppState.useTrack = 0;
                state = SPEED_SELECT;
            }
            break;
        case SPEED_SELECT:
            waitForVBlank();
            drawFullScreenImageDMA(speedSelection);
            if (KEY_JUST_PRESSED(BUTTON_A, currentButtons, previousButtons)) {
                currentAppState.fpn = 40;
                state = APP_FULLDRAW;
            } else if (KEY_JUST_PRESSED(BUTTON_B, currentButtons, previousButtons)) {
                currentAppState.fpn = 32;
                state = APP_FULLDRAW;
            } else if (KEY_JUST_PRESSED(BUTTON_RIGHT, currentButtons, previousButtons)) {
                currentAppState.fpn = 20;
                state = APP_FULLDRAW;
            }
            break;
        case APP_FULLDRAW:
